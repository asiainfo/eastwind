...
public class EastWindClient {
	...
	public void setServerLoader(ServerLoader serverLoader) {
		this.serverRepository.setServerLoader(serverLoader);
	}
	...
	public <T> SmartInvoker<T> createSmartInvoker(String group, String name, TypeReference<T> typeReference) {
		checkStart();
		Type returnType = typeReference.getType();
		SmartInvocationHandler handler = new SmartInvocationHandler(group, serverRepository, returnType);
		return new SmartInvoker<T>(group, name, returnType, handler);
	}
