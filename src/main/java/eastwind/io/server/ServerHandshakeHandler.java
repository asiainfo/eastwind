...
public class ServerHandshakeHandler extends SimpleChannelInboundHandler<Handshake> {
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		handshake.setUuid(CommonUtils.UUID);
		handshake.setStep(serverHandshaker != null && serverHandshaker.isMultiStep() ? 1 : 3);
	...
	protected void channelRead0(ChannelHandlerContext ctx, Handshake msg) throws Exception {
		if (msg.getStep() == 2) {
			Map<String, Object> out = Maps.newHashMap();
			if (serverHandshaker != null) {
				serverHandshaker.handshake(msg.getApp(), ctx.channel(),
						Collections.unmodifiableMap(msg.getAttributes()), out);
			}
			Handshake handshake = new Handshake();
			handshake.setApp(app);
			handshake.setUuid(CommonUtils.UUID);
